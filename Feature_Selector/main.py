...
def extract_features(file_path):
    with open(file_path, 'r', encoding='utf-8', errors='ignore') as file:
        ...
        if email_content_match:
            email_content = email_content_match.group(1).strip()
        else:
            # If "Content:" is not found, extract everything after the first empty line
            email_content_match = re.search(r'\n\s*\n(.*)', content, re.DOTALL)
